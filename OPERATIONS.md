...
Для account-flow есть Playwright-сценарий `npm run smoke:production`. Он получает
URL, два временных 2FA-пользователя и уникальный cleanup-маркер только через
переменные окружения. Пользователей и записи с маркером нужно создать перед
запуском и гарантированно удалить после него, даже если тест завершился ошибкой.
