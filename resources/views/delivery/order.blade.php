<x-delivery.site :title="'Order ' . $payload['order_number']">
	<div class="mx-auto max-w-5xl"
		...
		data-delivered-art="{{ asset('site/mock/banner-delivered.webp') }}"
		data-processing-art="{{ asset('site/mock/banner-processing.webp') }}">

		{{-- Top bar --}}
		<div class="mb-4 flex items-center justify-between gap-3">
			<a href="https://difmark.com/en/profile/GlobalGames" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-[#5b3df5]">
		...
		{{-- Status banner --}}
		<div class="relative overflow-hidden rounded-3xl border border-white bg-gradient-to-r from-[#efeafb] via-[#e2d8fa] to-[#cdbef5] p-6 shadow-sm sm:p-8">
			<div class="relative z-10 max-w-md">
				<p class="text-sm font-semibold text-[#5b3df5]/80">Your order is</p>
				...
				<p id="bannerEta" class="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[#5b3df5]">
			...
			<img id="bannerArt" src="{{ asset('site/mock/banner-processing.webp') }}" alt=""
		...
		<section class="mt-6 hidden rounded-3xl border border-white bg-white p-6 shadow-sm" id="connectionCard">
			...
			<form id="connForm" class="mt-4 hidden">
				...
				<input id="connection_code" name="connection_code" maxlength="8" minlength="6" autocomplete="off" autocapitalize="characters" spellcheck="false"
					...
					class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm uppercase tracking-[0.12em] outline-none focus:border-[#5b3df5] focus:ring-2 focus:ring-[#ddd2f7]">
				...
				<button type="submit" id="connSubmit" class="mt-3 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#7c5cff] to-[#4627ee] px-5 py-3 text-sm font-bold text-white shadow-lg shadow-blue-200 transition hover:opacity-95 disabled:opacity-60">
					<span class="btn-label">Send code</span>
				</button>
			</form>

			<div class="mt-4 hidden items-start gap-2 rounded-xl border border-[#cdbef5] bg-[#efe9fc] px-3 py-2.5 text-sm text-[#5b3df5]" id="connProgress">
		...
		<div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
			...
			<div class="rounded-3xl border border-white bg-white p-6 shadow-sm">
				...
				<ul class="mt-3 space-y-3 text-sm text-slate-600">
					...
					<li class="flex items-start gap-2"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="mt-0.5 h-5 w-5 shrink-0 text-[#5b3df5]"><circle cx="12" cy="12" r="9"/><path d="M12 16v-4M12 8h.01"/></svg><span><b class="text-slate-900">Having issues?</b><br>Contact our support team, we are here to help.</span></li>
				</ul>
				<a href="https://difmark.com/en/profile/GlobalGames" target="_blank" rel="noopener" class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#7c5cff] to-[#4627ee] px-4 py-3 text-sm font-bold text-white shadow-md">
	...
	<script>
		(() => {
			...
			function renderStepper(status) {
				...
				STEPS.forEach((name, i) => {
					...
					circle.className = 'flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold ' +
						(done ? 'bg-[#5b3df5] text-white' : active ? 'bg-[#5b3df5] text-white ring-4 ring-[#ddd2f7]' : 'bg-slate-100 text-slate-400');
			...
			function tabDot(status) {
				...
				if (status === 'waiting_for_connection_code') return 'bg-[#9b87ff]';
				if (['connection_failed', 'locked_24h'].includes(status)) return 'bg-rose-500';
				if (['connection_code_submitted', 'operator_connecting', 'waiting_for_operator', 'new'].includes(status)) return 'bg-[#5b3df5]';
				return 'bg-slate-400';
			}
			...
			function renderTabs() {
				...
				items.forEach((it, i) => {
					...
					b.className = 'inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition ' +
						(active ? 'border-transparent bg-gradient-to-r from-[#7c5cff] to-[#4627ee] text-white shadow' : 'border-slate-200 bg-white text-slate-700 hover:border-[#b3a0ee]');
