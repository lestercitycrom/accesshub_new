<x-delivery.site title="How it works">
	<div class="mx-auto max-w-4xl">
		...
		<div class="text-center">
			<span class="inline-flex items-center gap-2 rounded-full border border-[#cfe3fb] bg-white/70 px-3 py-1 text-xs font-semibold text-[#0b6bff] shadow-sm">Help center</span>
		...
		<div id="how" class="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
			...
			@foreach($steps as [$n, $t, $d])
				<div class="rounded-2xl border border-white bg-white p-5 shadow-sm">
					<div class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-[#0b6bff] to-[#0b6bff] text-sm font-bold text-white">{{ $n }}</div>
		...
		{{-- Guarantee --}}
		<div id="guarantee" class="mt-12 scroll-mt-24 overflow-hidden rounded-3xl border border-white bg-gradient-to-r from-[#eaf6ff] to-[#d6e9ff] p-6 shadow-sm sm:p-8">
			<div class="flex items-start gap-4">
				...
				<div>
					...
					<a href="https://difmark.com/en/profile/GlobalGames" target="_blank" rel="noopener" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#0b8dff] to-[#005be0] px-4 py-2.5 text-sm font-bold text-white shadow-md">Contact the seller</a>
				</div>
			</div>
		</div>

		<div class="mt-10 text-center">
			<a href="{{ route('delivery.take-order') }}" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#0b8dff] to-[#005be0] px-5 py-3 text-sm font-bold text-white shadow-lg shadow-blue-200">
				Track your order
